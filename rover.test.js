...
test('collisionDetection should return true if proposed position is outside of plateauBoundary', () => {
  const tempRover = Rover(
    { startX: 0, startY: 0, endX: 5, endY: 5 },
    { x: 3, y: 3, o: 'E' },
    cardinals
  );
  const returnedBoolean = tempRover.collisionDetection({ x: 10, y: 10 });
  expect(returnedBoolean).toStrictEqual(true);
});

test('receiveCommand should return report before moving outside of plateauBoundary', () => {
  const tempRover = Rover(
    { startX: 0, startY: 0, endX: 5, endY: 5 },
    { x: 3, y: 3, o: 'E' },
    cardinals
  );
  const returnedPosition = tempRover.receiveCommand('MMMMMMMMMMMM');
  expect(returnedPosition).toBe('5 3 E');
});
